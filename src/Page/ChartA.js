...
function ChartA() {
  ...
  const getAverageData = (data) => {
    ...
    Object.entries(data).forEach(([_, value]) => {
      ...
      if (value.Temperature !== "nan") {
        timeGroups[timeKey].temperatureValues.push(
  ...
  const temperatureData = {
    ...
    datasets: [
      {
        label: "Temperature (°C)",
        data: dataAverage.map((data) =>
          data.temperature === "nan" ? null : parseFloat(data.temperature)
        ),
    ...
    datasets: [
      {
        label: "Humidity (%)",
        data: dataAverage.map((data) =>
          data.humidity === "nan" ? null : parseFloat(data.humidity)
        ),
    ...
    datasets: [
      {
        label: "PPM",
        data: dataAverage.map((data) =>
          data.ppm === "nan" ? null : parseFloat(data.ppm)
        ),
